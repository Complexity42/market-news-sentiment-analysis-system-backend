...
def getBreakingviewsData():
    ...
    chrome.get("https://www.reuters.com/breakingviews")
    ...
    for item in soup.select("#main-content > div:nth-child(6) > ul > li"):
        urls.append("https://www.reuters.com" + item.select("a")[0]["href"])

    for item in soup.select(
        "#main-content > div:nth-child(7) > div > div:nth-child(1) > div > ul > li"
    ):
        urls.append("https://www.reuters.com" + item.select("a")[0]["href"])
    ...
    for url in urls:
        result = getModelByReutersPostUrl(url, source_name="Reuters - Breakingviews")
        # pp.pprint(result)
    ...
    # jsonFile = open("output_breakingviews.json", "w")
